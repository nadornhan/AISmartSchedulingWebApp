...
import { CheckIcon, ChevronRightIcon, CloseIcon, EditIcon, PlusIcon } from '../layout/icons';
...
function calendarTimeLabel(task: CalendarTask) {
  return `${task.scheduledStart ? 'Scheduled' : 'Due'} ${formatTime(task.calendarDate)}`;
}
...
function toCalendarTask(task: TaskResponse): CalendarTask | null {
  const calendarDate = task.scheduled_start ?? task.due_date;
  ...
  return {
    ...
    scheduleLocked: task.schedule_locked,
...
export function CalendarPage() {
  ...
  const refreshCalendarTasks = useCallback(
    async (signal?: AbortSignal) => {
      try {
        ...
        const rangeStart = new Date(visibleRange.dueFrom).getTime();
        const rangeEnd = new Date(visibleRange.dueTo).getTime();
        ...
        setCalendarTasks(
          items
            ...
            .filter((task) => {
              const timestamp = new Date(task.calendarDate).getTime();
  ...
  const visibleTasks = useMemo(
    () =>
      calendarTasks
        ...
        .sort(
          (first, second) =>
            new Date(first.calendarDate).getTime() - new Date(second.calendarDate).getTime(),
        ),
  ...
  async function moveTaskToDate(taskId: string, date: Date) {
    ...
    if (task.scheduleLocked) {
      setActionError('Unlock this task before changing its schedule.');
      return;
    }

    const currentDate = new Date(task.calendarDate);
    ...
    setMutatingTaskId(task.id);
    try {
      const updated = await rescheduleTask(task.id, payload);
      const nextTask = toCalendarTask(updated);
      setCalendarTasks((current) =>
        nextTask ? current.map((item) => (item.id === task.id ? nextTask : item)) : current,
      );
      selectDate(date);
    } catch (error) {
  ...
  function openRescheduleDialog(task: CalendarTask) {
    ...
    setRescheduleDate(dateKey(currentDate));
    setRescheduleTime(
  ...
  async function submitReschedule() {
    ...
    try {
      const payload =
        rescheduleTarget.scheduledStart && rescheduleTarget.scheduledEnd
          ? {
              scheduled_start: nextDate.toISOString(),
              scheduled_end: new Date(
                nextDate.getTime() +
                  new Date(rescheduleTarget.scheduledEnd).getTime() -
                  new Date(rescheduleTarget.scheduledStart).getTime(),
              ).toISOString(),
            }
          : { due_date: nextDate.toISOString() };
      const updated = await rescheduleTask(rescheduleTarget.id, payload);
      const nextTask = toCalendarTask(updated);
      setCalendarTasks((current) =>
        nextTask
          ? current.map((item) => (item.id === rescheduleTarget.id ? nextTask : item))
          : current,
      );
      selectDate(nextDate);
  ...
  return (
    ...
    <section className="hidden gap-5 lg:grid xl:grid-cols-[minmax(0,1.75fr)_minmax(380px,0.9fr)]">
      <div className="rounded-[var(--radius-lg)] border border-dashboard-border bg-dashboard-surface/65 shadow-panel">
        <div className="flex flex-col gap-3 border-b border-dashboard-border p-4 sm:p-5 lg:flex-row lg:items-center lg:justify-between">
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.7fr)_minmax(340px,0.95fr)]">
        ...
        {calendarView !== 'day' ? (
          <div className="grid grid-cols-7 border-b border-dashboard-border bg-dashboard-bg/20">
            {weekdayLabels.map((label) => (
              <div
                className="border-r border-dashboard-border px-3 py-3 text-sm font-semibold text-dashboard-muted last:border-r-0"
                key={label}
              >
                {label}
              </div>
...
function CalendarMainView({
  ...
  if (calendarView === 'week') {
    return (
      <div className="grid min-h-[calc(100vh-17rem)] grid-cols-7">
        {days.map((day) => (
          <CalendarDayCell
            day={day}
...
function DayViewTaskCard({
  ...
  return (
    <article
      className={cn(
        'group rounded-xl border p-4 shadow-[0_12px_32px_rgba(0,0,0,0.14)] transition hover:-translate-y-0.5 hover:shadow-[0_16px_38px_rgba(0,0,0,0.22)] sm:p-5',
        priorityClasses(task),
        task.scheduleLocked ? 'cursor-default' : 'cursor-grab active:cursor-grabbing',
        isMutating && 'cursor-wait opacity-60',
      )}
      draggable={!isMutating && !task.scheduleLocked}
    ...
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex min-w-0 items-start gap-4">
          <time className="shrink-0 rounded-lg border border-current/20 bg-dashboard-bg/45 px-3 py-2 text-base font-semibold text-current">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2 sm:justify-end">
          <button
            ...
            className="grid h-10 w-10 place-items-center rounded-lg border border-dashboard-border-strong bg-dashboard-bg/60 text-dashboard-muted transition hover:border-dashboard-accent/70 hover:text-dashboard-accent"
            disabled={isMutating}
            onClick={() => onReschedule(task)}
            title="Edit due date"
            ...
            disabled={isMutating || task.scheduleLocked}
            onClick={() => onReschedule(task)}
            title={task.scheduleLocked ? 'Unlock this task before rescheduling' : undefined}
            type="button"
          >
          ...
          <button
            ...
            type="button"
          >
            <span className="grid h-[18px] w-[18px] place-items-center rounded border border-current/50">
              {task.status === 'done' ? <CheckIcon className="h-3.5 w-3.5" /> : null}
            </span>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-current/10 pt-3">
        <span className="rounded-full border border-current/20 bg-dashboard-bg/35 px-3 py-1.5 text-sm font-semibold text-current">
          {priorityLabel}
        </span>
        {task.durationMinutes !== null ? (
          <span className="rounded-full border border-dashboard-border bg-dashboard-bg/35 px-3 py-1.5 text-sm font-medium text-dashboard-muted">
            {formatDurationLabel(task.durationMinutes)} estimated
          </span>
        ) : null}
        <span className="ml-auto text-sm font-medium text-dashboard-muted">
        ...
        <span className="ml-auto text-xs font-medium text-dashboard-muted">
          {task.scheduledStart && task.scheduledEnd
            ...
            : `Due ${formatTime(task.calendarDate)}`}
        </span>
...
function CalendarDayCell({
  ...
  return (
    <button
      className={cn(
        'flex h-[128px] flex-col border-r border-t border-dashboard-border bg-dashboard-bg/10 p-3 text-left transition last:border-r-0 hover:bg-dashboard-surface/65 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-dashboard-accent/50',
        variant === 'week' && 'h-auto min-h-[520px]',
      ...
      <div className="flex h-8 shrink-0 items-start justify-between">
        <span
          className={cn(
            ...
            day.isToday &&
              'grid h-8 min-w-8 place-items-center rounded-full bg-dashboard-accent text-dashboard-bg',
          )}
        >
          {day.date.getDate()}
        </span>
...
function CalendarTaskCard({
  ...
  return (
    <div
      className={cn(
        'rounded-md border px-3 py-2.5 shadow-[0_10px_28px_rgba(0,0,0,0.18)]',
        priorityClasses(task),
        task.status === 'done' && 'opacity-70',
        task.scheduleLocked ? 'cursor-default' : 'cursor-grab',
      )}
      draggable={!task.scheduleLocked}
      onDragEnd={(event) => event.currentTarget.classList.remove('opacity-50')}
      onDragStart={(event) => {
        if (task.scheduleLocked) return;
        event.stopPropagation();
  ...
  return (
    <aside className="rounded-[var(--radius-lg)] border border-dashboard-border bg-dashboard-surface/65 p-5 shadow-panel">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-sm font-semibold uppercase tracking-[0.12em] text-dashboard-accent">
            Daily agenda
          </p>
          <h2 className="mt-1 truncate text-2xl font-medium tracking-[-0.02em] text-dashboard-text">
            {formatSelectedDate(selectedDate)}
          </h2>
          <p className="mt-1 text-sm text-dashboard-muted">
            {tasks.length} {tasks.length === 1 ? 'scheduled task' : 'scheduled tasks'}
          <p className="mt-1 text-xs text-dashboard-muted">
            {tasks.length} {tasks.length === 1 ? 'calendar item' : 'calendar items'}
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <button
            aria-label="Previous day"
            className="grid h-10 w-10 place-items-center rounded-lg border border-dashboard-border bg-dashboard-raised text-dashboard-muted transition hover:border-dashboard-accent/60 hover:text-dashboard-accent"
          ...
          <button
            className="h-10 rounded-lg border border-dashboard-border bg-dashboard-raised px-3.5 text-base font-medium text-dashboard-muted transition hover:border-dashboard-accent/60 hover:text-dashboard-text"
            onClick={onToday}
            type="button"
          >
            Today
      ...
      <label className="mt-6 flex w-fit items-center gap-3 text-sm font-medium text-dashboard-muted">
        <button
...
function AgendaTask({
  ...
  return (
    <article
      className="grid cursor-grab grid-cols-[66px_16px_minmax(0,1fr)] items-center gap-3 active:cursor-grabbing"
      draggable={!isMutating}
      className={cn(
        'grid grid-cols-[58px_14px_minmax(0,1fr)] items-center gap-3',
        task.scheduleLocked ? 'cursor-default' : 'cursor-grab active:cursor-grabbing',
      )}
      draggable={!isMutating && !task.scheduleLocked}
      onDragEnd={(event) => event.currentTarget.classList.remove('opacity-50')}
      ...
      title={task.scheduleLocked ? 'Unlock this task before rescheduling' : calendarTimeLabel(task)}
    >
      <time className="text-right text-sm font-semibold tabular-nums tracking-[-0.01em] text-dashboard-muted">
        {formatTime(task.dueDate)}
      <time className="text-right text-xs font-semibold tabular-nums tracking-[-0.01em] text-dashboard-muted">
        {formatTime(task.calendarDate)}
      </time>
      <span
        className="relative z-10 h-3 w-3 justify-self-center rounded-full shadow-[0_0_0_4px_var(--bg-surface),0_0_12px_currentColor]"
        style={{ backgroundColor: task.projectColor }}
      />
      <div
        className={cn(
          'rounded-xl border bg-dashboard-bg/35 p-5 shadow-[0_10px_28px_rgba(0,0,0,0.12)] transition hover:-translate-y-0.5 hover:bg-dashboard-raised/75 hover:shadow-[0_14px_34px_rgba(0,0,0,0.2)]',
          priorityClasses(task),
        )}
      >
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-current/10 pt-3">
          <span className="inline-flex min-w-0 items-center gap-2 text-sm font-medium text-dashboard-muted">
            <span
          ...
          <span className="rounded-full border border-current/20 bg-dashboard-bg/35 px-2.5 py-1 text-xs font-semibold uppercase tracking-wide text-current">
            {priorityLabel}
          </span>
          <div className="ml-auto flex shrink-0 items-center gap-2">
            {task.durationMinutes !== null ? (
              <span className="text-sm font-semibold tabular-nums text-dashboard-muted">
                {formatDurationLabel(task.durationMinutes)}
              </span>
            ) : null}
            <button
              aria-label={`Edit due date for ${task.title}`}
              className="grid h-8 w-8 place-items-center rounded-md text-dashboard-muted transition hover:bg-dashboard-bg/70 hover:text-dashboard-accent"
              disabled={isMutating}
              onClick={() => onReschedule(task)}
              title="Edit due date"
              type="button"
          ...
          <span className="text-[10px] font-semibold uppercase tracking-wide text-dashboard-muted">
            {task.scheduledStart ? 'Scheduled' : 'Deadline'}
          </span>
          {task.durationMinutes !== null ? (
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <h2 className="text-xl font-semibold text-dashboard-text" id="reschedule-dialog-title">
              Reschedule task
            </h2>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <label className="block">
            <span className="mb-2 block text-sm font-medium text-dashboard-text">
              {task.scheduledStart ? 'Schedule date' : 'Due date'}
            </span>
            <input
