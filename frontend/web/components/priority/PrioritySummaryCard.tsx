  ...
  return (
    <article className="rounded-xl border border-dashboard-border bg-dashboard-surface/80 px-7 py-8 shadow-[inset_0_1px_0_rgba(255,255,255,0.025)]">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-base font-semibold text-dashboard-text">{column.title}</h2>
      </div>
      <div className="mt-3 flex items-end gap-3">
        <span className="text-4xl font-medium leading-none text-dashboard-text">{column.tasks.length}</span>
        <span className="pb-0.5 text-base text-dashboard-muted">tasks</span>
      </div>
      <p className="mt-4 text-base font-medium" style={{ color: column.accent }}>
        {column.description}
      </p>
    </article>
  );
...
function FlagIcon({ className }: { className: string }) {
  return (
    <svg aria-hidden="true" className={`h-6 w-6 fill-current ${className}`} viewBox="0 0 24 24">
