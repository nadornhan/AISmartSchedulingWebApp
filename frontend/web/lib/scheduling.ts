...
export function startFocusSession(input: {
  ...
  return apiRequest<FocusSessionDetail>('/scheduling/focus-sessions/start', {
    method: 'POST',
    body: JSON.stringify(input),
  });
}
...
export function updateFocusSession(
  ...
  return apiRequest<FocusSessionDetail>(`/scheduling/focus-sessions/${sessionId}`, {
    method: 'PATCH',
    body: JSON.stringify(input),
  });
}
...
export function finishFocusSession(
  ...
  return apiRequest<FocusSessionDetail>(
    `/scheduling/focus-sessions/${sessionId}/${completed ? 'complete' : 'cancel'}`,
    {
      method: 'POST',
...
export function getActiveFocusSession(signal?: AbortSignal) {
  return apiRequest<FocusSessionDetail | null>('/scheduling/focus-sessions/active', { signal });
}

export function listFocusSessions(limit = 50, signal?: AbortSignal) {
  return apiRequest<FocusSessionDetail[]>(
    `/scheduling/focus-sessions/history?limit=${limit}`,
    { signal },
  );
}
